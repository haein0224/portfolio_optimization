...
results_30 <- results_w_las <- results_w_17 <- matrix(0,lenmonth,ncol(data1)-1)  
...
for (month in 1:lenmonth) {
  ...
  ww <- foreach(lambda=iter(lambda, by='column'), .combine=cbind,.packages = c("quadprog","CVXR")) %dopar% { #, .export=c("proxSortedL1", "proxZ")) %dopar% { #, .combine=cbind) {
    ...
    j <- 0
    tau <- 10^(-5) 
    pi <- p # 상대위험회피정도 : pi > 0 (여기서는 임의의 값으로 두었음,,)
...
for ( i in 3) { #:7) {
  port <- port_name[i-2]
  risk <- # 숏폴 비모수 추정치 사용 # 표준편차로 risk 사용하는 경우: round(sd(results[-1,i]),4)
