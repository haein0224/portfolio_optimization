...
for (month in 1:lenmonth) {
  ...
  ww <- foreach(lambda=iter(lambda, by='column'), .combine=cbind) %dopar% { #, .export=c("proxSortedL1", "proxZ")) %dopar% { #, .combine=cbind) {
    ...
    j <- 0
    tau <- 10^(-5) 
    pi <- 10 # 상대위험회피정도 : pi > 0 (여기서는 임의의 값으로 두었음,,)
...
for ( i in 3:5) { #:7) {
  port <- port_name[i-2]
  risk <- round(sd(results[-1,i]),4) # 숏폴 비모수 추정치 사용 # 표준편차로 risk 사용하는 경우: round(sd(results[-1,i]),4)
...
dt_result_D <- cbind(dt_result_D,c(round(turnover(results_30),4), NA, NA)) #, round(turnover(results_w_17),4), round(turnover(results_w_las),4),NA, NA))
dt_result_D <- cbind(dt_result_D, c(round(setdiff(results_30),4), NA, NA)) #, round(setdiff(results_w_17),4), round(setdiff(results_w_las),4), NA, NA))
dt_result_D <- cbind(dt_result_D, c(round(mean(rowSums(results_30 !=0)),4), NA, p)) #, round(mean(rowSums(results_w_17 !=0)),4), round(mean(rowSums(results_w_las !=0)),4), NA, p))
