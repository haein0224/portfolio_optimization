# new_dataprep
# 기존 데이터프렙함수는 세팅만 도와주고 수익률을 돌아가는 타이밍 마다 새로 계산해 사용했음
# -> 새로운 함수에서는 전체 데이터에 대해 수익률로 계산된 셋을 아웃풋으로 만들어서 이 과정을 줄이고자함
...
lassoPenalty <- function(w,lamb) lamb*p_norm(w-w_tilde,1)
...
cl = makeCluster(n_core-4) # 사용가능한 코어중 하나를 빼놓고 할당
cl
...
lenmonth <- 2 #12*5 # 몇 년치 데이터를 다룰 것인지 : 10년치
...
results_30 <- results_w_17 <- results_w_las <- matrix(0,lenmonth,ncol(data1)-1)
...
for (month in 1:lenmonth) {
  ...
  ww <- foreach(lambda=iter(lambda, by='column'), .combine=cbind,.packages = c("quadprog","CVXR")) %dopar% { #, .export=c("proxSortedL1", "proxZ")) %dopar% { #, .combine=cbind) {
    ...
    while (TRUE) {
      #w <- solve( c(eta/2)*(II + e%*%t(e) + rb%*%t(rb) + R%*%t(R) )) %*% ( c(eta)*(v+e+c(rp)*rb +R%*%z) - (rb + alpha + c(beta)*e + c(theta)*rb - R%*%gamma))
      if (month != 1) {
  ...
  # w_idx <- which(max(colMeans(t(R)%*%round(ww[,-ncol(ww)],2)))==colMeans(t(R)%*%round(ww[,-ncol(ww)],2)))
  ...
  if (length(w_idx) >= 2 & month >=2) {
    ...
    w_idx <- sample(w_idx,1) # 랜덤하게 하나 결정 (이게 맞나..?)
  ...
  return_lasso <- N_daily_return[-ncol(data1)]%*%rep(ww[1,17],p)
